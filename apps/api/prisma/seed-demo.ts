 ...
 * Exécuter explicitement avec : pnpm db:seed:demo
 *
 * Le mot de passe commun est fourni via DEMO_SEED_PASSWORD.
 */
import {
  PrismaClient,
  Prisma,
...
async function main(): Promise<void> {
  ...
  if (!DEMO_PASSWORD) {
    throw new Error('DEMO_SEED_PASSWORD doit être défini pour lancer le seed de démonstration.');
  }
  ...
  const pVis = await prisma.project.create({
    data: {
      ...
      description:
        "Outil interne de gestion de projets agile inspiré d'Azure DevOps (Backlog, Kanban, Sprints, Git PR).",
      company: 'VisioraAI',
  ...
  const pEcom = await prisma.project.create({
    data: {
      ...
      description:
        'Refonte de la marketplace B2B avec catalogue produits en temps réel et intégration ERP SAP.',
  ...
  const pAI = await prisma.project.create({
    data: {
      ...
      description:
        'Microservice RAG et classification sémantique de contrats par modèles LLM et vision.',
  ...
  const repo1 = await prisma.repository.create({
    data: {
      ...
      name: 'visiora-agile',
      description:
        'Dépôt principal du monorepo Visiora Agile (Frontend React 19 & Backend NestJS).',
  ...
  const sprint1 = await prisma.sprint.create({
    data: {
      ...
      retroSummary:
        'Excellente vélocité sur le socle technique initial. Découpage Zod partagé très apprécié.',
    },
  });
  ...
  await prisma.retrospectiveItem.createMany({
    data: [
      {
        sprintId: sprint1.id,
        category: RetroCategory.WENT_WELL,
        content: 'Monorepo Turborepo + pnpm très rapide',
        authorId: users['dev1@visiora.ai'].id,
      },
      {
        sprintId: sprint1.id,
        category: RetroCategory.WENT_WELL,
        content: 'Conception RBAC par projet sans faille',
        authorId: users['po@visiora.ai'].id,
      },
      {
        sprintId: sprint1.id,
        category: RetroCategory.TO_IMPROVE,
        content: 'Les critères d’acceptation doivent être validés avant le sprint planning',
        authorId: users['dev2@visiora.ai'].id,
      },
      {
        sprintId: sprint1.id,
        category: RetroCategory.ACTION_ITEM,
        content: 'Automatiser la génération du client TypeScript côté frontend',
        isDone: true,
  ...
  await prisma.milestone.createMany({
    data: [
      {
        projectId: pVis.id,
        name: 'Livraison MVP Socle Technique',
        description: 'Auth, BDD, monorepo et architecture validés',
        date: new Date('2026-08-14'),
        isReached: true,
      },
      {
        projectId: pVis.id,
        name: 'Bêta Interne Équipe & Kanban',
        description: 'Ouverture du Task Board à toute l’équipe de dév',
      ...
      },
      {
        ...
        description: 'Finalisation des rapports et durcissement DevSecOps',
        date: new Date('2026-09-20'),
        isReached: false,
      },
      {
        projectId: pVis.id,
        name: 'Mise en Production Officielle',
        description: 'Déploiement sur cluster Kubernetes sécurisé',
  ...
  const epicBoard = await prisma.workItem.create({
    data: {
      ...
      description:
        'Glisser-déposer accessible entre 5 colonnes, filtres d’URL dynamiques et réordonnancement sans collision.',
  ...
  const epicGit = await prisma.workItem.create({
    data: {
      ...
      description:
        'Déclaration des branches, liaison PR ↔ tickets, passage au statut "Prêt pour approbation" et validation PO.',
      status: WorkItemStatus.IN_PROGRESS,
      priority: Priority.HIGH,
  ...
  const epicReports = await prisma.workItem.create({
    data: {
      ...
      description:
        'Suivi de vélocité par sprint, calcul du burndown quotidien et indicateurs de productivité d’équipe.',
  ...
  const stLogin = await prisma.workItem.create({
    data: {
      ...
      description:
        'Formulaire de connexion avec validation Zod, stockage du token en mémoire et refresh token httpOnly.',
      technicalNotes:
        'Bcrypt 10 rounds, cookie SameSite=Lax, protection contre la concurrence de rotation.',
      status: WorkItemStatus.DONE,
      priority: Priority.CRITICAL,
      storyPoints: 5,
    ...
    data: [
      {
        ...
        content: 'Le token d’accès est conservé en mémoire vive uniquement',
        position: 0,
        isMet: true,
      },
      {
        workItemId: stLogin.id,
        content: 'Le cookie de rafraîchissement porte l’attribut httpOnly',
        position: 1,
        isMet: true,
      },
      {
        workItemId: stLogin.id,
        content: 'Un compte inactif est rejeté avec un message clair',
        position: 2,
        isMet: true,
      },
    ],
  ...
  const stRBAC = await prisma.workItem.create({
    data: {
      ...
      description:
        'L’écran Repos permet au PO de voir les PRs en attente, de visualiser la branche cible et de valider la livraison.',
    ...
    data: [
      {
        ...
        content: 'Seul le Product Owner du projet peut approuver ou demander des changements',
        position: 0,
        isMet: true,
      },
      {
        workItemId: stRBAC.id,
        content: 'Historique des transitions enregistré dans PullRequestEvent',
        position: 1,
        isMet: true,
      },
      {
        workItemId: stRBAC.id,
        content: 'Notification envoyée à l’auteur lors du changement de statut',
        position: 2,
        isMet: false,
      },
    ],
  ...
  const stDnD = await prisma.workItem.create({
    data: {
      ...
      description:
        'Intégration @dnd-kit avec mise à jour optimiste du cache React Query et retour en arrière en cas d’erreur réseau.',
      technicalNotes:
        'Calcul du boardRank via l’API POST /work-items/:id/move avec beforeId et afterId.',
      status: WorkItemStatus.IN_PROGRESS,
      priority: Priority.HIGH,
      storyPoints: 13,
  ...
  const stFilters = await prisma.workItem.create({
    data: {
      ...
      description:
        'Barre de filtres persistée dans les query params de l’URL pour permettre le partage direct de liens.',
  ...
  await prisma.workItem.create({
    data: {
      ...
      description:
        'Upload sécurisé avec restriction MIME (PDF, PNG, JPG, ZIP), max 25 Mo et stockage sur bucket objet S3.',
      status: WorkItemStatus.TODO,
      priority: Priority.MEDIUM,
      storyPoints: 8,
      parentId: epicBoard.id,
  ...
  await prisma.workItem.create({
    data: {
      ...
      description:
        'Lorsqu’un membre est supprimé d’un projet, ses tickets conservent son ID et provoquent une erreur ASSIGNEE_NOT_MEMBER.',
      status: WorkItemStatus.TODO,
      priority: Priority.CRITICAL,
      storyPoints: 3,
      isBlocked: true,
      blockedReason:
        'Nécessite la validation métier sur le comportement souhaité (désassigner automatiquement ou alerter).',
      sprintId: sprint2.id,
      assigneeId: users['dev2@visiora.ai'].id,
      reporterId: users['sm@visiora.ai'].id,
      rank: rankAt(14),
  ...
  await prisma.workItem.create({
    data: {
      ...
      description:
        'Le calcul rolledUpPoints doit agréger les sous-tâches aussi bien sous les stories que sous les bugs.',
      status: WorkItemStatus.IN_PROGRESS,
      priority: Priority.HIGH,
      storyPoints: 2,
      sprintId: sprint2.id,
...
- Tests unitaires et d'intégration validés`,
    ...
    data: [
      {
        ...
        actorId: users['dev2@visiora.ai'].id,
        fromStatus: null,
        toStatus: PullRequestStatus.OPEN,
        comment: 'PR ouverte avec tests unitaires complets.',
      },
      {
        pullRequestId: pr1.id,
        actorId: users['dev2@visiora.ai'].id,
        fromStatus: PullRequestStatus.OPEN,
        toStatus: PullRequestStatus.READY_FOR_APPROVAL,
        comment: 'Code review demandée à @Amel Ben Salah.',
      },
    ],
  });
    ...
    data: [
      {
        ...
        actorId: users['dev1@visiora.ai'].id,
        fromStatus: null,
        toStatus: PullRequestStatus.OPEN,
        comment: 'Initial PR',
      },
      ...
      {
        pullRequestId: pr2.id,
        actorId: users['po@visiora.ai'].id,
        fromStatus: PullRequestStatus.READY_FOR_APPROVAL,
        toStatus: PullRequestStatus.APPROVED,
        comment: 'Approuvé par le Product Owner.',
      },
    ],
  });
  ...
  const pr3 = await prisma.pullRequest.create({
    data: {
      ...
      reviewComment:
        'Attention au cas où la valeur est vide : l’URL ne doit pas afficher "?status=" inutilement. Merci de corriger.',
    },
  });
    ...
    data: [
      {
        ...
        fromStatus: null,
        toStatus: PullRequestStatus.OPEN,
        comment: 'Création du correctif.',
      },
      {
        pullRequestId: pr3.id,
        actorId: users['po@visiora.ai'].id,
        fromStatus: PullRequestStatus.OPEN,
        toStatus: PullRequestStatus.CHANGES_REQUESTED,
        comment: 'Modifications demandées sur le nettoyage des URLs vides.',
      },
    ],
  });
  ...
  await prisma.activityLog.createMany({
    data: [
      ...
      },
      {
        ...
        action: 'assigned',
        field: 'assignee',
        newValue: users['dev1@visiora.ai'].name,
      },
      {
        projectId: pVis.id,
        entityType: EntityType.SPRINT,
        entityId: sprint2.id,
        actorId: users['sm@visiora.ai'].id,
  ...
  console.log(
    `  Colonnes Board      : Toutes garnies (TODO, IN_PROGRESS, IN_TEST, READY_FOR_APPROVAL, DONE)`,
  );
